...
【❗ 换行硬约束（最重要 — qwen 经常违反）】
- 表格 header 行、separator 行、每一个 data 行**必须各自独占一行**，每行末尾换行
- 表格 header 和 separator 之间**不允许出现空行**（出现空行会破坏 CommonMark 表格识别）
- 标题（### / ##）之后**必须紧跟换行**再开始正文/列表
- 有序列表的每一项（1. 2. 3.）**必须各自独占一行**，绝不能用空格分隔多个项
- 无序列表的每一项（- xxx）**必须各自独占一行**
- 水平分割线 \`---\` **必须独占一行**，前后各留空行
- 严禁把多行表格、多个列表项、标题+内容用空格挤在同一行`,
