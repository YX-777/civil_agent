...
interface DashboardClientProps {
  ...
  initialRange: "week" | "month" | "all";
}

export default function DashboardClient({ initialData, initialRange }: DashboardClientProps) {
  const [timeRange, setTimeRange] = useState<"week" | "month" | "all">(initialRange);
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`/api/stats?range=${timeRange}&userId=${DEFAULT_USER_ID}`);
        if (!response.ok) {
